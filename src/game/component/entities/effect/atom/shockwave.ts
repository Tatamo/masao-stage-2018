...
export class ShockWaveEffect extends Entity {
	...
	constructor(
		...
		super(level, x, y);
		this.sprite = new PIXI.Sprite(level.api.resource.images["shockwave"]);
		...
		*update(): IterableIterator<void> {
			...
			for (let i = 0; i <= timespan; i++) {
				this.parent.sprite.scale.x =
					this.parent.width / 64 + ((this.parent.width_end - this.parent.width) / 64) * easing(i / timespan);
				this.parent.sprite.scale.y =
					this.parent.height / 64 +
					((this.parent.height_end - this.parent.height) / 64) * easing(i / timespan);
